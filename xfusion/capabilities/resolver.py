...
def resolve_intent_to_capability(
    ...
    system_prompt = f"""You are an intent classifier for XFusion, a secure system operations agent.
...
def _fallback_keyword_matching(user_input: str) -> tuple[str | None, dict[str, Any] | None, str | None]:
    ...
    # Disk operations
    if "disk" in user_input_lower and ("clean" in user_input_lower or "full" in user_input_lower):
        return "disk.check_usage", {"path": "/"}, None
    elif "disk" in user_input_lower or "磁盘" in user_input_lower or "空间" in user_input_lower:
        return "disk.check_usage", {"path": "/"}, None
    ...
    # Process operations
    elif "list processes" in user_input_lower or ("processes" in user_input_lower and "port" not in user_input_lower):
        ...
        if any(word in user_input_lower for word in ("stop", "kill")):
            # This would need multi-step planning
            return "process.find_by_port", {"port": port}, None
        return "process.find_by_port", {"port": port}, None
    
    # User operations
    elif "create user" in user_input_lower or "create a new user" in user_input_lower or "create new user" in user_input_lower:
        import re
        # Handle "Create user alice", "Create a new user alice", "Create new user alice"
        match = re.search(r"(?:create\s+(?:a\s+)?(?:new\s+)?user)\s+(\S+)", user_input, re.IGNORECASE)
        if match:
